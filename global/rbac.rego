...
allow {
	...
	country := data.global.users[input.user].location.country
	country == "US"
}
...
user_is_admin {
	...
	data.global.users[input.user].roles[i] == "admin"
}
...
user_is_granted[permission] {
	...
	role := data.global.users[input.user].roles[i]

	# `permission` assigned a single permission from the permissions list for 'role'...
	permission := data.global.role_permissions[role][j]
}
